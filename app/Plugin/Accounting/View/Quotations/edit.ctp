...
<?php 
	echo $this->Form->create('Quotation', array('action' => 'edit'));
?>
...
<?php
    ...
    echo $this->Form->inputDatepicker('date', array( 'label' => array('class' => 'col-lg-2 control-label', 'text' => 'Date' ), 'class' => 'form-control'));
	echo $this->Form->input('po_ref_number', array('label' => array('text' => 'PO/Ref Number', 'class' => 'col-lg-2 control-label')));
	echo $this->Form->input('tax', array('label' => array('text' => 'Tax (percentage)', 'class' => 'col-lg-2 control-label')));
...
<table class="table" id="tblProductService">
	...
	<tbody>
		...
		$subtotal = 0;
		$waiver = 0;
		if(!empty($this->request->data['QuotationDetail'])):
			foreach($this->request->data['QuotationDetail'] as $k => $item):
		?>
		<tr id="<?php echo $k;?>">
			<td class="col1">
				<input type="hidden" name="data[QuotationDetail][<?php echo $k;?>][id]" value="<?php echo $item['id'];?>" />
				<input type="hidden" name="data[QuotationDetail][<?php echo $k;?>][quotation_id]" value="<?php echo $item['quotation_id'];?>" />
				<select name="data[QuotationDetail][<?php echo $k;?>][product_service_id]" id="product_service" class="select2" onchange="change_product($(this))">
					<option value="">Select one</option>
					<?php
					if(!empty($product_service)) {
						foreach($product_service as $a => $vl) {
							$selected = '';
							if($item['product_service_id'] == $a)
								$selected = ' selected';
							echo '<option'.$selected.' value="'. $a .'">'. $vl .'</option>';
						}
					}
					?>
				</select>
			...
			<td class="col2"><div class="dv-populate"><textarea class="form-control" noeditor="true" rows="5" name="data[QuotationDetail][<?php echo $k;?>][description]"><?php echo $item['product_description'];?></textarea></div></td>
			<td class="col3">
				<input name="data[QuotationDetail][<?php echo $k;?>][quantity]" seperator="&lt;/div&gt;" class="form-control col_quantity" type="text" value="<?php echo $item['quantity'];?>" id="row_qty_0">
				<div class="displaynone error">Quantity is number and not empty.</div>
			</td>
			<td class="col4">
				<input name="data[QuotationDetail][<?php echo $k;?>][price]" seperator="&lt;/div&gt;" class="form-control col_price" type="text" value="<?php echo $item['price'];?>" id="row_price_0">
				<div class="displaynone error">Price is number and not empty.</div>
			</td>
			<td class="col5">
				<div class="amount_result"><?php echo number_format($item['quantity'] * $item['price'], 2);?></div>
				<input type="hidden" name="data[QuotationDetail][<?php echo $k;?>][waiver_status]" class="waiver_status" value="<?php echo $item['waiver_status'];?>" />
			</td>
			<td class="col6">
				<?php if($item['waiver_status'] == 1): 
					$subtotal += ($item['quantity'] * $item['price']);
				?>
				<a href="javascript:void(0)" onclick="apply_waiver($(this))" class="apply_waiver">Apply Waiver</a>
				<?php else: 
					$waiver += ($item['quantity'] * $item['price']);
				?>
				<a href="javascript:void(0)" onclick="remove_waiver($(this))">Remove waiver</a>
				<?php endif; ?>
			</td>
		</tr>
		<?php
...
<?php 
	echo $this->Form->input('subtotal', array('readonly' => true, 'default' => $subtotal));
	echo $this->Form->input('waiver', array('readonly' => true, 'default' => $waiver));
?>
<div class="form-group">
	<label for="QuotationDiscount" class="col-lg-2 control-label">Discount</label>
	<div class="col-lg-10">
		<input name="data[Quotation][discount]" seperator="&lt;/div&gt;" class="form-control" type="text" value="<?php echo $this->request->data['Quotation']['discount']?>" id="QuotationDiscount">
...
<?php
	echo $this->Form->input('total_tax', array('readonly' => true, 'default' => (($subtotal) * $this->request->data['Quotation']['tax'] / 100)));
	echo $this->Form->input('total', array('readonly' => true));
	echo $this->Form->input('payment_structure', array('type' => 'textarea'));
	echo $this->Form->input('late_payment', array('type' => 'textarea'));
	echo $this->Form->input('bank_information', array('type' => 'textarea'));
...
<script type="text/javascript">
	...
	$('#btnAddRow').click(function() {
		...
		var newRow='<tr id="tr_row_'+name+'">';
                ...
                newRow+='<td class="col2"><div class="dv-populate"><textarea class="form-control" noeditor="true" rows="5" name="data[QuotationDetail]['+ name +'][description]"></textarea></div></td>';
...
/*list function*/
	...
	function validate() {
		...
		$('#tblProductService tbody tr').each(function() {
			...
			if( parent_tr.children('.col1').children('select.select-ui-primary').val() == '') {
				flag = flag && false;
	...
	function apply_waiver(el) {
		BootstrapDialog.confirm('Please ...!', function(result){
			if(result) {
				tr_parent = el.parent().parent();
				// tr_parent.children('.col1').children('select').attr('disabled', true);
				tr_parent.children('.col1').children('div.select-ui-primary').children('ul').find('li').removeClass('disabled');
	...
	function remove_waiver(el) {
		BootstrapDialog.confirm('Please ...!', function(result){
			if(result) {
				tr_parent = el.parent().parent();
				// tr_parent.children('.col1').children('select').attr('disabled', false);
				tr_parent.children('.col1').children('div.select-ui-primary').children('ul').find('li').removeClass('disabled');
