	...
	if(!empty($data['SaveReport'])) {
		...
		foreach($data['SaveReport'] as $k => $item) {
			switch ($item['SaveReport']['type_report']) {
				case 'day_view':
					$link = $this->Html->link($item['SaveReport']['title'], array('plugin' => 'marketing', 'controller' => 'reports', 'action' => 'enquiries', '?' => array('type_report' => $item['SaveReport']['type_report'], 'start_date' => formatDate($item['SaveReport']['start_date']), 'end_date' =>  formatDate($item['SaveReport']['end_date']) )));
					break;
				case 'day_of_week':
					$link = $this->Html->link($item['SaveReport']['title'], array('plugin' => 'marketing', 'controller' => 'reports', 'action' => 'enquiries', '?' => array('type_report' => $item['SaveReport']['type_report'], 'day_of_week' => formatDate($item['SaveReport']['start_date']))));
					break;
				default:
					$link = $this->Html->link($item['SaveReport']['title'], array('plugin' => 'marketing', 'controller' => 'reports', 'action' => 'enquiries', '?' => array('type_report' => $item['SaveReport']['type_report'], 'time_of_day' => formatDate($item['SaveReport']['start_date']))));
					break;
			}
