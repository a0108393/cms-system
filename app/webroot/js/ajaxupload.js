...
(function($)
{
	...
	var fileObject = function(file, name, size, ext, AU)
	{
		...
		//hide upload button when auto start
			...
			if(this.readyState == 4 && this.status == 200)
			{
				try
				{
					...
					else if(isLast)
					{
						...
						me.onFinish(ret.name, ret.size, ret.status, ret.info, ret);
					}
					else
					{
    ...
    fileObject.prototype.uploadStandard = function(up_all)
    {
		...
		$('#ax-main-frame').unbind('load').bind('load', this, function(e){
			...
			try{
	    		...
	    		//get file info
	    		e.data.onFinish(ret.name, ret.size, ret.status, ret.info, ret);
			}catch(err){
	...
	fileObject.prototype.onError = function(err)
	{
		...
		this.AU.settings.error.call(this, err, this.name, this.AU.settings);
	...
	fileObject.prototype.onFinish = function(name, size, status, info, obj)
	{
    	this.name	= name;
    	...
    	this.onProgress(100);
    	this.uploadButton.removeClass('ax-abort').attr('onclick','BootstrapDialog.alert(\'This file uploaded. Could not upload again.\'); return false;');//remove abort button dont upload again
    	this.progressBar.width(0);
    	this.progressPer.html(_('File uploaded 100%')); 
    	this.AU.settings.success.call(this, name, obj, this.AU.settings);//call success method
    	...
    	for(var i = 0; i<this.AU.files.length; i++)
    	{
    		...
    		if(this.AU.files[i].status!=1 && this.AU.files[i].status!=-1){
				runFinish = false; 
				uploaded.push(obj);
			}
    	}
	...
	var AjaxUploader = function($this, settings)
	{
		...
		this.fieldSet  	= $('<fieldset class="ajaxupload" />').append('<legend class="ax-legend">' + _('Select Files') + '</legend>').appendTo($this);
		...
		if(settings.maxFiles <= 1){
			browse_title = _('Add file');
		}else{
			browse_title = _('Add files');
		}
		this.browse_c = $('<a class="'+bs_browse+'" title="' + browse_title + '" />').append('<span class="'+bs_b_icon+'"></span> <span class="ax-text">' + browse_title + '</span>').appendTo(this.fieldSet);
	...
	AjaxUploader.prototype.bindEvents = function($this)
	{
	    ...
	    if(this.hasAjaxUpload)
	    {
	    	...
	    	{
	    		this.fieldSet.find('.ax-legend').html(settings.title);
	    	}
	...
	AjaxUploader.prototype.finish = function()
	{
		...
		this.settings.finish.call(this, fileNames, this.files, this.settings);
	...
	AjaxUploader.prototype.addFiles = function(files)
	{
		...
		for (var i = 0; i < files.length; i++) 
		{
			...
			{
				this.settings.error.call(this, err, name, this.settings);
			}
		}
    ...
    var globalSettings = 
    {
    	...
    	success:		function(name, obj, settings){	onSuccessCallback(name, obj, settings); },				//function that triggers every time a file is uploaded
    	finish:			function(file_names, file_obj, settings){ onFinishCallback(file_names, file_obj, settings); },			//function that triggers when all files are uploaded
    	error:			function(err, fn, settings){ onErrorCallback(err, settings); },		//function that triggers if an error occuors during upload,
		...
		title: 			_('Select Files or Drag&Drop Files'),
		removeAllButton: true,
		errorBootstrapDialog: false
    };
    function onSuccessCallback(name, obj, settings){
		//add after hidden field
		if( typeof settings.successCallback != 'undefined' ){
			return eval(settings.successCallback);
		}
		var $el = $('#' + settings.elementId);
		$el.after('<input type="hidden" name="'+ settings.fieldName +'['+ name +']" value="'+ obj.path +'" />');
		
		//add after hidden field to prepare data move files ...
		$el.after('<input type="hidden" name="ajax_files_uploaded[]" value="'+ obj.path +'" />');
	};
	function onErrorCallback(err, settings){
		if(settings.errorBootstrapDialog){
			BootstrapDialog.alert(err);
		}else{
			alert(err);
		}
	}
	function onFinishCallback(file_names, file_obj, settings){
		if( typeof settings.finishCallback != 'undefined' ){
			return eval(settings.finishCallback);
		}
	};
	var methods =
	{
		...
		onFinishFlash: function(json, pos)
		{
			...
			try
			{
				...
				{
					AU.files[pos].onFinish(json_ret.name, json_ret.size, json_ret.status, json_ret.info, json_ret);
				}
			}
			catch(err)
			{
