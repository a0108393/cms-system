...
// ReSharper disable once InconsistentNaming
    DateTimePicker = function (element, options) {
		...
		fillDow = function () {
			...
			var startW = picker.options.startWeek;
		    if (pMoment()._lang._week.dow == 0) { // starts Week change by VietND
				while (startW < picker.options.startWeek + 7){
					html.append('<th class="dow">' + weekdaysMin[(startW++)%7] + '</th>');
				}
				//old code
		        /* for(i = 0; i < 7; i++) {
        ...
        fillDate = function () {
           ...
           prevMonth.date(days).startOf('week');
			...
			if((prevMonth.date() + picker.options.startWeek) <= 28){
				prevMonth.date(prevMonth.date() + picker.options.startWeek);
			}else{
				prevMonth.date(prevMonth.date() + picker.options.startWeek - 7);
			}
            ...
            while (prevMonth.isBefore(nextMonth)) {
				...
				if (prevMonth.weekday() === picker.options.startWeek) {  // viet added
                    row = $('<tr>');
                    html.push(row);
                }
